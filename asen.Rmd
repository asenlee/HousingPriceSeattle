...
ols <- function(train, test) {
  ...
  preds80 <- predict(mod, newdata = test, interval="predict", level=0.8)
  preds50 <- predict(mod, newdata = test, interval="predict", level=0.5)
  plot(mod$fitted.values, mod$residuals, ylim = c(-3000, 3000), pch = 16)
  return(list(model = mod, preds=preds,preds80 = preds80, preds50=preds50))
}
...
data <- test %>% select(-c(zip))
all_data <- cbind(data, olsMod$preds50)

#create plot
ggplot(all_data, aes(x = data$price, y = fit)) + #define x and y axis variables
...
```{r}
data <- test %>% select(-c(zip))
all_data <- cbind(data, wlsMod$preds80)

#create plot
ggplot(all_data, aes(x = beds, y = price)) + #define x and y axis variables
    geom_point() + #add scatterplot points
    stat_smooth(method = lm) + #confidence bands
    geom_line(aes(y = lwr), col = "coral2", linetype = "dashed") + #lwr pred interval
    geom_line(aes(y = upr), col = "coral2", linetype = "dashed") #upr pred interval
...
assess.glmnet(ridgeMod$preds, newy = test$price)
```
...
ridgePI50 <- add_pi(test, ridgeMod$model, names=c("lower", "upper"), alpha=0.25, nsims=20000)
ridgePI80 <- add_pi(test, ridgeMod$model, names=c("lower", "upper"), alpha=0.1, nsims=20000)
ridgePI80
```
...
lasso <- function(train, test) {
  ...
  mod <- glmnet(X, Y, alpha=1, lambda=cv_mod$lambda.min)
  
  newX <- test %>% select(-price, -zip) %>% as.matrix()
  preds <- predict(mod, s='lambda.min', newx=newX)
  plot(cv_mod)
  return(list(model = mod, preds = preds))
}
...
assess.glmnet(lassoMod$preds, newy = test$price)
```
...
IS50ols <- intervalScore(olsMod$preds50, test$price, 0.5)
IS80ols <- intervalScore(olsMod$preds80, test$price, 0.8)
...
IS50wls <- intervalScore(wlsMod$preds50, test$price, 0.5)
IS80wls <- intervalScore(wlsMod$preds80, test$price, 0.8)
