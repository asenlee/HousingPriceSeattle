...
pred_gbm <- function(training, testing) {
    mod   <- gbm(price ~ ., training)
    preds <- predict(mod, newdata = testing, n.trees = 100, type = "response")
    print("gggg")
    return(preds)
}

gbm <- function(training, testing) {
  nboot <- 100 # number of bootstrap samples
  predsBoot <<- replicate(nboot, {
    indices <<- sample(nrow(training), replace = TRUE)
    trainingD  <- training[indices, ]
    pred_gbm(trainingD, testing)}, simplify = TRUE)

  # calculate prediction intervals from bootstrapped predictions
  gbmPreds <- do.call(cbind, predsBoot)
  lower80    <- apply(gbmPreds, 2, quantile, probs = 0.1)
  upper80    <- apply(gbmPreds, 2, quantile, probs = 0.9)
  est80      <- apply(gbmPreds, 2, mean)
  preds80    <- data.frame(est80 = est80, lower80 = lower80, upper80 = upper80)
  
  lower50    <- apply(gbmPreds, 2, quantile, probs = 0.25)
  upper50    <- apply(gbmPreds, 2, quantile, probs = 0.75)
  est50      <- apply(gbmPreds, 2, mean)
  preds50    <<- data.frame(est50 = est50, lower50 = lower50, upper50 = upper50)
  
  mod        <- gbm(price ~ ., data = training)
  #plot_model(training, mod$fit, testing, preds50[, 1]) 
  return(list(model = mod, preds50 = preds50, preds80 = preds80))
}
gbmMod <- gbm(training, testing)
```
